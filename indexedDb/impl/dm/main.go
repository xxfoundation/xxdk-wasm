...
var dmCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		m.wtm.RegisterMessageChannelCallback(string(kv.PortOp),
			func(port js.Value, channelName string) {
				jww.INFO.Printf("[DM] Received KV port for channel: %s", channelName)

				kvStore, err := kv.NewWorkerThread(port, channelName)
				if err != nil {
					jww.ERROR.Printf("[DM] Failed to create KV WorkerThreadStore: %+v", err)
					return
				}
				...
				m.kvStore = kvStore
				jww.INFO.Print("[DM] KV WorkerThreadStore created successfully")
			})
